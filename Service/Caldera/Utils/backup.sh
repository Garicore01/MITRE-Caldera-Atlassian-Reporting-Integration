...
if ! systemctl list-unit-files | grep -q "caldera.service"; then
    echo "Caldera service not found. Please create caldera.service, enable and start it 
...
echo "[$(date)] Caldera backup created: ${BACKUP_DIR}/caldera_backup_${BACKUP_DATE}.tar.gz 
    Size: ${BACKUP_SIZE}" >> ${LOG_FILE}
# Keep only last 5 backups
cd ${BACKUP_DIR} && ls -t caldera_backup_*.tar.gz | tail -n +6 | xargs -r rm
echo "Caldera backup created: ${BACKUP_DIR}/caldera_backup_${BACKUP_DATE}.tar.gz. 
    Size: ${BACKUP_SIZE}. Last 5 backups kept."
